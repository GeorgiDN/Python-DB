...
def product_quantity_ordered():
    total_quantity_ordered = (
        Product.objects.annotate(total_ordered_quantity=Sum('orderproduct__quantity')).exclude(
...
def filter_products():
    ...
    available_products = Product.objects.filter(query).order_by("-price", "name")
...
def give_discount():
    ...
    query = Q(is_available=True) & Q(price__gt=3.00)
    available_products = Product.objects.filter(query).order_by("-price", "name")
    available_products.update(price=F('price') * decrease)
